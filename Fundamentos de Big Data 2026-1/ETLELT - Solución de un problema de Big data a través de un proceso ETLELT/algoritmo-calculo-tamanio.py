...
try:
    ...
    tiempo_total_ms = (fin_proceso - inicio_proceso) * 1000 # Convertimos a milisegundos
    # --- FIN MEDICIÓN DE TIEMPO ---

    print(f"Tiempo de procesamiento para {registros} registros: {tiempo_total_ms:.2f} ms")
    # -------------------------------------------------------------------------

    # -------------------------------------------------------------------------
    # Tamaño de la Base de datos "bigdata" y la Tabla "tamanio"
    # -------------------------------------------------------------------------
    command   = '''SELECT pg_size_pretty(pg_database_size('bigdataGrupo6'));'''
    resultado = cursor.execute(command)
    record    = cursor.fetchall()
